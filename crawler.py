...
"""
import urllib.request, re, ast, random, docopt
...
def main():
    ...
    watchlist = 'https://www.imdb.com/user/ur56869126/watchlist'
    markup = urllib.request.urlopen(watchlist)
    ...
    winner_titleblock = winner_soup.find('div', {'class': re.compile('TitleBlock__TitleMetaDataContainer.+')})
    winner['length'] = winner_titleblock.find_all('li', {'class': 'ipc-inline-list__item'})[2].text
    winner['image'] = winner_soup.find('img', {'class': 'ipc-image'})['src']
    winner['plot'] = winner_soup.find('span', {'class': re.compile('GenresAndPlot__TextContainerBreakpoint.+')}).text

    winner_genrechips = winner_soup.find_all('a', {'class': re.compile('GenresAndPlot__GenreChip.+')})
    winner['genre'] = [i.find('span', {'class': 'ipc-chip__text'}).text for i in winner_genrechips]

    winner_credits = winner_soup.find('div', {'class': re.compile('PrincipalCredits__PrincipalCredits.+')})
    winner_credits = winner_credits.find_all('li', {'class': 'ipc-metadata-list__item'})
    winner['credits']= {
        i.find(
            ['span', 'a'], 
    ...
    imdb_rating = winner_soup.find('span', {'class': re.compile('AggregateRatingButton__RatingScore.+')}).text
    meta_rating = winner_soup.find('span', {'class': 'score-meta'}).text
    winner['score'] = 'IMDb Score: ' + imdb_rating + '  Metacritic Score: ' + meta_rating
